...
addpath([base_dirr,'processed/3dhkl_scan/']);
addpath([base_dirr,'processed/3dhkl_scan/core/']);

% mask for bad pixels and gaps.
% pixels marked (as 1) in the mask will not be used in the reconstruction.
mask = load([base_dirr,'processed/mask_pilatus100k.mat']); mask=mask.mask_pilatus100k;
        ...
        try
            ...
            if skip_flag
                ...
                [Itot,ztot] = image2hkl(framenr,dirr,file,...
                                        0,x_size,y_size,...
                                        rot_ax,ub,dk,P,mask,hmax,hmin,kmax,kmin,...
                                        lmax,lmin,n_steps,bkg,int_corr);
                %         % Lorentz correction
                %         Int_total_t = Itot ./ double(ztot);              
                %         Int_total_t(isnan(Int_total_t)) = 0;
